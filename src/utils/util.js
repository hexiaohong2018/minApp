const formatTime = (date, isLongTime = true) => {
...
const getUrlParam = (url, key) => {
	...
	function analyseParam(key, value) {
		if (/=/.test(value) && external) {
			let reg4 = new RegExp('(?:([^' + split + ']*?)=([^' + split + ']*))' + split + '', 'g');
			let sub = [];
			...
			let reg5 = new RegExp('' + split + '(?:([^' + split + ']*?)=([^' + split + ']*))$', 'g');
	...
	if (result) {
		for (let i = 0; i < result.length; i++) {
			if (result[i].key == key) {
				paraValues = result[i].value;
			}
		}
	}
	return paraValues || result;
}
/**
 * 消息提醒的方法
 */
const showModalFn = (tipText, confirm, cancel) => {
	uni.showModal({
		content: tipText,
...
const phoneInfo = () => {
	return new Promise((resolve, reject) => {
		if (_phoneInfo) resolve(_phoneInfo);
		else {
			uni.getSystemInfo({
				success: ({
					model,
...
const isDeepColor = (_color) => {
	...
	return total >= 192 ? false : true;
}
//设置页码激活颜色

const setActiveColor = function(active_color, def_color) {
	return isDeepColor(active_color) ? active_color : def_color
}
...
// 	return I1;
// }
//时间差mss format(hh:mm:ss h:m:s)
function formatDuring(mss, format) {
	if (mss < 0) throw "时间差不能小于0";
	//1000 * 60 * 60 * 24 = 86400000//一天转毫秒
	// 1000 * 60 * 60=3600000 一小时转毫秒
	// 1000 * 60 = 60000 一分钟转毫秒
	var day = Math.floor(mss / 86400000),
		hours = Math.floor((mss % 86400000) / 3600000),
		minutes = Math.floor((mss % 3600000) / 60000),
		seconds = Math.floor((mss % 60000) / 1000);
...
const verify = (function() {
	...
	function tel(tel) {
		let reg = /^(\+86)?(13|14|15|17|18|19)[0-9]{9}$/;
		return reg.test(tel);
	};
...
function decodeWXCodeParams(options) {
	let params = {};
...
//timestamp 时间戳 如果不给定默认为获取当前系统时间戳
//days 获取天数，默认4天，表示已当前timestamp为开始，共获取的天数
function getWeek({ timestamp, days = 4 } = {}) {
  var result = [],
    mon = 0,
    date = 1,
    time = null,
    ...
    week = "";
  timestamp = isToday ? new Date().getTime() : timestamp;
  for (var i = 0; i < days; i++) {
    ...
    switch (time.getDay()) {
      case 0:
        week = isToday ? (i == 0 ? '今天' : i == 1 ? '明天' : i == 2 ? '后天' : '周日') : '周日';
        result.push({ week, day: mon + '-' + date });
        break;
      case 1:
        week = isToday ? (i == 0 ? '今天' : i == 1 ? '明天' : i == 2 ? '后天' : '周一') : '周一';
        result.push({ week, day: mon + '-' + date });
        break;
      case 2:
        week = isToday ? (i == 0 ? '今天' : i == 1 ? '明天' : i == 2 ? '后天' : '周二') : '周二';
        result.push({ week, day: mon + '-' + date });
        break;
      case 3:
        week = isToday ? (i == 0 ? '今天' : i == 1 ? '明天' : i == 2 ? '后天' : '周三') : '周三';
        result.push({ week, day: mon + '-' + date });
        break;
      case 4:
        week = isToday ? (i == 0 ? '今天' : i == 1 ? '明天' : i == 2 ? '后天' : '周四') : '周四';
        result.push({ week, day: mon + '-' + date });
        break;
      case 5:
        week = isToday ? (i == 0 ? '今天' : i == 1 ? '明天' : i == 2 ? '后天' : '周五') : '周五';
        result.push({ week, day: mon + '-' + date });
        break;
      case 6:
        week = isToday ? (i == 0 ? '今天' : i == 1 ? '明天' : i == 2 ? '后天' : '周六') : '周六';
        result.push({ week, day: mon + '-' + date });
        break;
    }
