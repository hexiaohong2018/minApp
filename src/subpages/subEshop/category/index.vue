        ...
        <scroll-view class="category" scroll-y>
          <view v-if="subCategorys.length > 0" class="category-content">
            <view
              ...
              :key="item.productsubcategory_id"
              @click="showProducts(categorys[activeIndex].productcategory_id,item.productsubcategory_id)"
            >
              <view class="img"></view>
...
export default {
  ...
  methods: {
    onChange(index) {
      productList
        .getProductCategory(this.categorys[index].productcategory_id)
        .then(res => {
          if (res.length == 0) {
            uni.navigateTo({
              url:
                "../../eshop/productList/index?id=" +
                this.categorys[index].productcategory_id
            });
          } else {
  ...
  onLoad() {
    productList.getProductCategory().then(res => {
      this.categorys = res;
      productList
        .getProductCategory(this.categorys[this.activeIndex].productcategory_id)
        .then(res => {
          this.subCategorys = res;
        });
    });
