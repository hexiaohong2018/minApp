	...
	<view class="index-container">
		<nav-bar :title="title" :background="navbarColor" clear-float>
			<template v-slot:left>
				<view v-if="name == 'discover'" @click="mode = mode == 0 ? 2 : 0" class="iconfont-dc" :class="'icon-dc-' + (mode == 2 ? 'liebiao' : 'app')" style="font-size: 23px;"></view>
			</template>
		</nav-bar>
...
import { decodeWXCodeParams, phoneInfo } from '../../utils/util.js';
...
export default {
	...
	onLoad(options) {
		...
		//推荐人ID
		this.recommend_member_id = params.rid;

		if (this.recommend_member_id && this.recommend_member_id > 0) {
			uni.setStorageSync('recommend_member_id', this.recommend_member_id);
			share
				.getIncentiveInfo()
				.then(res => {
