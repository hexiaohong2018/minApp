<template>
	<view class="index-container" v-show="show">
		<view v-if="isOpenSubShop" class="shops" :style="{ backgroundColor: color, color: _color }">
			...
			<view class="iconfont-vant icon-vant-qr" style="font-size:55rpx"></view>
		</view>
		<mescroll-uni
			@init="mescrollInit"
			:up="{ auto: false }"
...
export default {
		...
		this.onLoad = (function() {
			...
			return function() {
				if (is_first_run) {
					...
					this.navHeight = store.getters.navHeight + (shopInfo.shopList ? 40 : 0);
					this.tabBarHeight = store.getters.tabBarHeight;
					
					
						// distributor_name = loginInfo.distributor_name || '',
						// sales_user_id = store.getters.salesUserId,
	...
	methods: {
		...
		upCallBack(mescroll) {
			if (this.uiconfig) {
				...
				productList.getProductPageList({ page: mescroll.num }).then(res => {
		...
		init() {
			...
			//当开启分店模式时，需要获取用户位置
			// this.data.is_open_sub_shop && addr.location({ wxFn: wx.getLocation,success:function(res){
			//   console.log(res);
			// },fail:function(msg){
			//   console.log(msg);
			...
			// distributor_name,
			// shop_name,
			// notice: shopInfo.sv_shop_notice,
			// is_open_sub_shop: shopList ? true : false
			// })
			// return Promise.resolve();
			// } else {
			//   if (loginInfo) {
			// let distributor_name = loginInfo.distributor_name || "";
			// let sv_advertising_picture = shopInfo.sv_advertising_picture || [];
			// this.setData({
			//   uiconfig,
			//   distributor_name,
			//   shop_name,
			//   ad_img: sv_advertising_picture.map(item => ({
			//     img: item.code
			//   })),
			//   is_open_sub_shop: shopList ? true : false
			// })
			// }
			// return Promise.all([productList.getProductCategory(), productList.getProductPageList(true)]).then(values => {
			//   let [category, products] = values;
			//   var arr = [];
			//   for (let i = 0; i < category.length && i < 5; i++) {
			//     let obj = {};
			//     obj["des"] = category[i].sv_pc_name;
			//     obj["link"] = {
			//       "target": "/subPages/category/index",
			//       "params": {
			//         "id": category[i].productcategory_id
			//       }
			//     }
			//     arr.push(obj);
			//   }
			...
			//   this.setData({
			//     navItems: arr,
			//     product_loadState: products.isAll ? 2 : 1,
	...
	computed: {
		...
		isOpenSubShop() {
			return (this.$store.getters['loginInfo/shopInfo'].shopList) ? true : false;
		}
	}
};
</script>
